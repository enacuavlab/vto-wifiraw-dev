...
int main(int argc, char *argv[]) {
  ...
  int8_t offsetraw;
  int16_t fd_tun_udp;
  uint8_t udp[UDP_SIZE], role, *ptr;
  uint16_t fd[FD_NB],fd_out[FD_NB],id,cpt,maxfd=0,maxdev,dev,ret,seq=1,seqprev;
  struct sockaddr_in addr, addr_in[FD_NB], addr_out[FD_NB];
  ...
  if (role) addr_in[dev].sin_addr.s_addr = inet_addr(ADDR_REMOTE_BOARD); 
  else addr_in[dev].sin_addr.s_addr = inet_addr(ADDR_REMOTE_GROUND); 
  ...
  if (role) addr_out[dev].sin_addr.s_addr = inet_addr(ADDR_REMOTE_GROUND);
  else addr_out[dev].sin_addr.s_addr = inet_addr(ADDR_REMOTE_BOARD);
  offsetraw = 0;
#endif
  ...
  if (role) strcpy(ifr.ifr_name,"airtun"); else strcpy(ifr.ifr_name, "grdtun");
  ...
  if (role) addr.sin_addr.s_addr = inet_addr(addr_str_tunnel_board);
  else addr.sin_addr.s_addr = inet_addr(addr_str_tunnel_ground);
  ...
  if (role) addr.sin_addr.s_addr = inet_addr(addr_str_tunnel_ground);
  else addr.sin_addr.s_addr = inet_addr(addr_str_tunnel_board);
  memcpy(&ifr.ifr_addr,&addr,sizeof(struct sockaddr));
  ...
  if (role) {        // option on board    
    if (role==2) {   // option with telemetry (one bidirectional link)
      if (-1 == (fd[dev]=open("/dev/ttyAMA0",O_RDWR | O_NOCTTY | O_NONBLOCK))) exit(-1);
      struct termios tty;
...
//            uint8_t *dump=udp+radiotapvar+sizeof(ieeehdr)+sizeof(payhdr_t)+sizeof(subpayhdr_t);printf("read(%ld)\n[",lensum-sizeof(subpayhdr_t));for(int i=0;i<lensum-sizeof(subpayhdr_t);i++)printf(" %x ",dump[i]);printf("]\n");
	    ...
	    if (crcok) {
  	        ...
  	        if ((role) || (id == 1)) {
                  ...
                  len = sendto(fd[id], ptr + sizeof(subpayhdr_t), len, 0, (struct sockaddr *)&(addr_out[id]), sizeof(struct sockaddr));
  	        }

//                uint8_t *dump=ptr+sizeof(subpayhdr_t);printf("write(%d)(%ld)\n[",id,len);for(int i=0;i<len;i++)printf(" %x ",dump[i]);printf("]\n");
...
//            uint8_t *dump=ptr+sizeof(subpayhdr_t);printf("read(%d)(%ld)\n[",cpt,len);for(int i=0;i<len;i++)printf(" %x ",dump[i]);printf("]\n");
 ...
 //       uint8_t *dump=udp+sizeof(radiotaphdr)+sizeof(ieeehdr)+sizeof(payhdr_t)+sizeof(subpayhdr_t);printf("write(0)(%ld)\n[",lensum-sizeof(subpayhdr_t));for(int i=0;i<lensum-sizeof(subpayhdr_t);i++)printf(" %x ",dump[i]);printf("]\n");
