...
int main(int argc, char *argv[]) {
  ...
  for(;;) {
	...
	if (id == 0) {                                     // read raw and send to udp and/or uart
          len_in = read(px.fd[0], udp_in, PKT_SIZE_1_IN);
	  printf("In RAW(%ld)\n\n",len_in);
          if (len_in > 0) {
            ...
            if (~crc != crc_rx) totfails ++;
              ...
              if (lastpkt)  {
		if (id == 1) len_out = write(px.fd[1], udp_out, offset);  // On Board and Ground write Tunnel (udp)
		else {
		...
		printf(" ]\n\n");
		   ...
		   // send to raw 
          len_in = read(px.fd[id], udp_in + offset1, UDP_SIZE - offset1);
