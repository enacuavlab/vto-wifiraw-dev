...
int main(int argc, char *argv[]) {
  ...
  uint32_t len_udp = 0;
  uint32_t udp_size = 65507;
  uint8_t buf_udp[udp_size];
  pu8 = buf_udp; 
  ...
  for(;;) { 
    ...
    if(FD_ISSET(fd_raw, &readset)) {  // Less CPU consumption than pcap_loop()
      ...
      if ( n == 1 ) {
        ssize_t bytes = read( fd_raw, packetBuffer, PKT_SIZE_1 );
	  ...
	  memcpy(buf_udp + len_udp,ppay,len);
	  len_udp += len;

	  lg += sprintf(log+lg, "(%ld)",stp_n);
...
*/
	...
	if (len < DATA_SIZE) {
	  ...
	  if (port!=0) r = sendto(fd_out,buf_udp,len_udp,0,(struct sockaddr *)&addr, sizeof(struct sockaddr_in));
	  else write(fd_out, buf_udp, len_udp);

	  printf("[%d]%s\n",len_udp,log);fflush(stdout);
	  lg=0;
	  len_udp=0;
        }
      }
