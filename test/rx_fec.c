...
/*
            ...
            for (i=0;i<param_fec_packets_per_block;i++) inpkts[i]=malloc(MAX_USER_PACKET_LENGTH);
            unsigned char *outpkts[param_data_packets_per_block - param_fec_packets_per_block];
            for (i=0;i<param_fec_packets_per_block;i++) outpkts[i]=malloc(MAX_USER_PACKET_LENGTH);
            ...
            for(int i=0; i < param_fec_packets_per_block; i++) {
              if(rx_ring[ring_idx].fragment_map[i]) {
                inpkts[i] = rx_ring[ring_idx].fragments[i];
                inpkts[i] = data_pkgs[i];
                indexes[i] = i;
              } else {
                for(;j < param_data_packets_per_block; j++) {
                  if(rx_ring[ring_idx].fragment_map[j]) {
                    inpkts[i] = rx_ring[ring_idx].fragments[j];
                    outpkts[ob_idx++] = rx_ring[ring_idx].fragments[i];
                    inpkts[i] = data_pkgs[j];
                    outpkts[ob_idx++] =data_pkgs[i]; 
                    indexes[i] = j;
                    j++;
                    break;
                  }
		...
		while ((n = ieee80211_radiotap_iterator_next(&rti)) == 0) {
		    ...
		    */
			...
			case IEEE80211_RADIOTAP_DBM_ANTSIGNAL:
				rx_status->adapter->current_signal_dbm = (int8_t)(*rti.this_arg);
				break;
...
//	rx_status = status_memory_open();
	rx_status = malloc (sizeof(rx_status));
/*
	int j = 0;
	int x = optind;
	...
	for(int i=0; i<param_block_buffers; ++i)
	{
        ...
        for(;;) {
          ...
          if(FD_ISSET(interface.selectable_fd, &readset)) {
             process_packet(&interface, block_buffer_list);
	  }
	}
}
