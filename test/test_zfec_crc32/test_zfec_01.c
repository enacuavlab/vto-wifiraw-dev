...
int main(int argc, char *argv[]) {
  ...
  uint8_t PKT_DATA = 1, fec_k = 4, fec_n = 8; // fec_n = original data + extra data (fec_k)
  ...
  dec_in[1] = data_frame[1];  // primary (if i present must be at index i)
  dec_in[2] = data_frame[2];  // primary (if i present must be at index i)
  dec_in[3] = fec_frame[0];   // secondary block  (anywhere), but in same order as encode output

  unsigned indexes[] = {0,1,2,4};  // (index[row] >= code->k) || (index[row] == row)
  ...
  frame_out[1] = data_frame[1];
  frame_out[2] = data_frame[2];
  frame_out[3] = dec_out[0];
